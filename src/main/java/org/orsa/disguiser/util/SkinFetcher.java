package org.orsa.disguiser.util;
...
//            Logging.error(e.getMessage());
    ...
    public static Optional<Property> fetchSkinByUUID(UUID uuid) {
//        Logging.debug("Fetching Mojang skin of UUID: " + uuid);

        String trimmedUuid = uuid.toString().replace("-", "");

//        Logging.debug(trimmedUuid);

        try {
            String reply = urlRequest(URI.create("https://sessionserver.mojang.com/session/minecraft/profile/" + trimmedUuid + "?unsigned=false").toURL(), true, null);
            return getSkinFromReply(reply);
        } catch (IOException e) {
//            Logging.error(e.getMessage());
        }
...
//        Logging.debug("Parsing skin reply: " + reply);
        ...
        return Optional.of(new Property(DisguisedPlayer.PROPERTY_TEXTURES, value, signature));
    }
